...
def debug_plugin_status_view(req) -> Response:
    ...
    if req.method == "POST":
        ...
        try:
            if req.form.get("clear_log"):
                ...
                try:
                    with open(os.path.join(logs_dir, "debug_plugin_errors.log"), "a", encoding="utf-8") as ferr:
                        ferr.write(f"[{datetime.utcnow()}] Cleared debug log via POST\n")
                except Exception:
                    pass
                flash("Debug log cleared.", "success")
                # Add cache-busting param to force browser to reload
                import time
                return redirect(url_for("ui.plugin_status_view", plugin=plugin, _cb=int(time.time())))
            else:
                # Log unexpected POST without clear_log
                try:
                    with open(os.path.join(logs_dir, "debug_plugin_errors.log"), "a", encoding="utf-8") as ferr:
                        ferr.write(f"[{datetime.utcnow()}] POST to status but no clear_log in form: {dict(req.form)}\n")
                except Exception:
                    pass
            ...
            try:
                with open(os.path.join(logs_dir, "debug_plugin_errors.log"), "a", encoding="utf-8") as ferr:
                    ferr.write(f"[{datetime.utcnow()}] Error clearing log: {type(e).__name__}: {e}\n")
            except Exception:
                with open(orchestrator_log, "a", encoding="utf-8") as ferr:
                    ferr.write(f"[{datetime.utcnow()}] [DEBUG_PLUGIN] Error clearing log: {type(e).__name__}: {e}\n")
    ...
    try:
        ...
        entries = entries[-100:]
        pretty = "\n\n".join(json.dumps(e, indent=2, default=str) for e in entries) if entries else "No debug logs found."
        # Filter form
        filter_form = f'''
      ...
      '''
        status_html = filter_form + f"<pre style='background:#f4f8fd;border-radius:6px;padding:1.2em;font-size:1.07em;overflow-x:auto;'>{pretty}</pre>"
    except Exception as e:
