...
def _calculate_sentiment_values(
    rsi, macdhist, ultosc, slowk, slowd
    ...
    if latest_hist is None or (
        isinstance(latest_hist, float) and np.isnan(latest_hist)
    ):
        macd_value = 0
    elif latest_hist < 0:
        macd_value = 25
    ...
    if (
        latest_slowk is not None
        and latest_slowd is not None
        and not np.isnan(latest_slowk)
        and not np.isnan(latest_slowd)
    ):
...
def calculate(df, params) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    rsi, upper, middle, lower, macd, macdhist, ultosc, slowk, slowd = (
        _calculate_indicators(df)
    )
    rsi_value, macd_value, ultimate_osc_value, stochastic_value = (
        _calculate_sentiment_values(rsi, macdhist, ultosc, slowk, slowd)
    )
    bb_value = 0  # Not well defined, set to 0
    avg_sentiment = round(
        (rsi_value + bb_value + macd_value + ultimate_osc_value + stochastic_value) / 5
    )
