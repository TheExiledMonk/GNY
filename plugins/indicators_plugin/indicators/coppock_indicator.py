...
def calculate(df, params) -> Tuple[Dict[str, Any], Dict[str, int]]:
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        roc1[roc_1:] = np.where(
            valid1, (prices[roc_1:] - prices[:-roc_1]) / prices[:-roc_1], np.nan
        )
        roc2[roc_2:] = np.where(
            valid2, (prices[roc_2:] - prices[:-roc_2]) / prices[:-roc_2], np.nan
        )
    ...
    return {"coppock": float(latest) if latest is not None else None}, {
        "wma_1": wma_1,
        "roc_1": roc_1,
        "roc_2": roc_2,
    }
