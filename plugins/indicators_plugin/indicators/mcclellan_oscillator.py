...
def calculate(df, params):
    ...
    if len(data) < max(period, ema_period):
        return {'mcclellan_oscillator': None}, {'period': period, 'ema_period': ema_period}
    ...
    return {'mcclellan_oscillator': float(latest) if latest is not None and not np.isnan(latest) else None}, {'period': period, 'ema_period': ema_period}
