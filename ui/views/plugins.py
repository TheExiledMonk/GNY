...
def _render_plugin_config(mod: Any, plugin: str) -> str:
    ...
    return render_template(
        ...
        message=message,
        config_html=mod.handle_config_request(
            request, config, None, message, config_manager
        ),
    )
...
def plugin_config_view(plugin: str) -> str:
    ...
    if plugin == "gather_plugin":
        ...
        return handle_config_request(
            request, config, pipeline, message, plugin_config_repo
        )
    ...
    return (
        f"<h2>Plugin '{plugin}' does not provide handle_config_request(). No config UI available.</h2>",
        501,
    )
